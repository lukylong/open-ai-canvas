...
func TestBundledProviderCatalogKeepsLegacyComfyUIWorkflow(t *testing.T) {
	center, err := newPluginRuntime(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	catalog := (&Service{pluginRuntime: center}).PluginProviderCatalog(string(protocol.SurfaceAdminSystemChannel), string(protocol.CapabilityVideo), false)
	for _, item := range catalog {
		if item.ID != "comfyui-workflow" {
			continue
		}
		if !containsPluginCapability(item.Categories, protocol.CapabilityImage) || !containsPluginCapability(item.Categories, protocol.CapabilityVideo) {
			t.Fatalf("ComfyUI workflow categories = %#v", item.Categories)
		}
		return
	}
	t.Fatal("legacy ComfyUI workflow missing from administrator catalog")
}
