...
// legacyComfyUIWorkflowAdapter keeps the external comfy-adapter protocol in the
// unified provider catalog. Execution remains owned by the existing host path.
func legacyComfyUIWorkflowAdapter() Adapter {
	info := Metadata{
		ID: "comfyui-workflow", Version: "1.0.0", Name: "ComfyUI 工作流", Vendor: "Canvas",
		Categories: []Capability{CapabilityImage, CapabilityVideo}, Scopes: allSurfaces(),
		Create: "POST /v1/jobs", Poll: "GET /v1/jobs/{job_id}", ContentType: "application/json",
		Enabled: true, Installable: true,
	}
